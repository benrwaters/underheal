...
local function updateDistributionChannel()
	local gameVersion = type(HEALBOT_GAME_VERSION) == "number" and HEALBOT_GAME_VERSION or build
	local instanceCategory = LE_PARTY_CATEGORY_INSTANCE
	local hasLFGRestrictions = HasLFGRestrictions and HasLFGRestrictions()
    if gameVersion > 2 and ((instanceCategory and (IsInGroup(instanceCategory) or IsInRaid(instanceCategory))) or instanceType == "pvp" or instanceType == "arena" or hasLFGRestrictions) then
